import { decompressDeep } from '../lib';
...
describe('decompressDeep - core', () => {
    it('should decompress a short string compressed 3 times', () => {
        const str = JSON.stringify({
            widget: {
                debug: 'on',
                window: {
        ...
        compressed = compress(`[${compressed},${compressed}]`);
        compressed = compress(`[${compressed},${compressed}]`);

        expect(decompressDeep(compressed)).toStrictEqual(`[${`[${str},${str}]`},${`[${str},${str}]`}]`);
    });
});
